...
escapedValue = takeTill (\x -> x `elem` (s "; \0\r\n")) <?> "escaped value"
...
middle :: Parser Param
middle = Param <$> takeTill1 (\x -> x `elem` (s " \0\r\n")) <?> "middle"

trailing :: Parser ByteString
trailing = takeTill (\x -> x `elem` (s "\0\r\n")) <?> "trailing" 
        
user :: Parser User
user = User <$> takeTill (\x -> x `elem` (s " @\0\r\n")) <?> "user"
